...
export type DeleteUserMutation = { delete_user_by_pk?: Maybe<Pick<User, 'email'>> };
...
export type GetUserQuery = { user_by_pk?: Maybe<Pick<User, 'email' | 'avatar' | 'username'>>, user_aggregate: { aggregate?: Maybe<Pick<User_Aggregate_Fields, 'count'>> } };
...
export type SubscribeToUserListSubscription = { user: Array<(
    Pick<User, 'role' | 'email' | 'avatar' | 'username' | 'session_last_updated' | 'created_at' | 'opaque_id'>
    & { preference?: Maybe<Pick<User_Preference, 'dark_mode'>> }
  )> };
